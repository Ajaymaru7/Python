...
vending_machine = {"Chips": "0A", "Soda": "0B", "Chocolate": "0C",
                   "Fruit Snacks": "0D"}  # We created this hash (python calls them dictionaries) to list the items and their selection
prices = {"Chips": "$0.75", "Soda": "$0.50", "Chocolate": "$1.25",
          "Fruit Snacks": "$.25"}  # We created this hash to have the same keys as the first hash, but this time having prices be their values
...
def purchase(
        needed_money):  # Made this for personal ease. It simply keeps track of money after checking to see if you can buy said item.
    global money
...
def transaction(
        user_input):  # Function is made to take care of choosing said item, it then calls the "purchase" funciton
    global money
    if user_input == "0A":
        purchase(0.75)
    elif user_input == "0B":
